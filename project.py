...
def make_card(img_path, title, tab, col):
    with tab :
        with col :
            ip = f"""
                  
                <div class="card" onclick="window.open('./dum.html', '_blank')" style="width: 98%; height : 98%; border : 1px solid black; text-align:center; box-shadow: 4px 4px 8px 0px rgba(0, 0, 0, 0.2), 6px 6px 20px 0px rgba(0, 0, 0, 0.19);" >
                    ...
                    <button onclick="window.open('https://google.com', '_blank')">Know More</button>
                     </div>
                </div>
                 
                """
            # if st.components.v1.html(ip, width=300, height=365):
            #     print("clicked", title)
    ...
    for title in project_dict.keys():
        # try :
            if project_dict[title]['categaory'] == 'Robotics':
                make_card(project_dict[title]['img'], title, t_robotics, cols[1][count[1]%num_columns] )
                count[1] += 1
            
            elif project_dict[title]['categaory'] == 'Electronics/Control Systems':
                make_card(project_dict[title]['img'], title,t_vlsi, cols[2][count[2]%num_columns]  )
                count[2] += 1
            
            elif project_dict[title]['categaory'] == 'Software Development':
                make_card(project_dict[title]['img'], title,t_software, cols[3][count[3]%num_columns]  )
                count[3] += 1
            ...
            make_card(project_dict[title]['img'], title,t_all, cols[0][count[0]%num_columns] )
            count[0] += 1
